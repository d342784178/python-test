...
class ThreadPoolExecutorWithQueueSizeLimit(ThreadPoolExecutor):
    ...
    def __init__(self, max_workers=None, *args, **kwargs):
        ...
        self._work_queue = queue.Queue(1)
...
class M3u8Download:
    ...
    def __init__(self, url, name, max_workers=64, num_retries=5, base64_key=None):
        ...
        self.get_m3u8_info(self._url, 10)
    ...
    def get_m3u8_info(self, m3u8_url, num_retries):
        ...
        """
        try:
            with requests.get(m3u8_url, timeout=(2, 5), verify=False, headers=self._headers) as res:
    ...
    def download_ts(self, ts_url, name, num_retries):
        ...
        try:
            if not os.path.exists(name):
                with requests.get(ts_url, stream=False, timeout=(2, 5), verify=False, headers=self._headers) as res:
    ...
    def download_key(self, key_line, num_retries):
        ...
        try:
            with requests.get(true_key_url, timeout=(2, 5), verify=False, headers=self._headers) as res:
                with open(os.path.join(self._file_path, 'key'), 'wb') as f:
                    f.write(res.content)
